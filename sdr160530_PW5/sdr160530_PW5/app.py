...
def addItem():  
    try:
        if session.get('user'):
            ...
            cursor.execute("INSERT INTO tbl_todo(title, description, userid, completed) VALUES (%s, %s, %s, false)", (_title, _description, _user))
...
@app.route('/editeItems', methods=['GET','POST'])
def editItems():
    try:
        if session.get('user'):
            _user = session.get('user')
